...
const Dealer = () => {
  ...
  const get_dealer = async () => {
    ...
    console.log(retobj); // Debugging line to check the API response
      ...
      <div style={{ marginTop: "10px" }}>
        {dealer ? (
          <>
            ...
            <h4 style={{ color: "grey" }}>
              {dealer?.city || "City not available"},
              {dealer?.address || "Address not available"}, Zip -{" "}
              {dealer?.zip || "N/A"},{dealer?.state || "State not available"}
            </h4>
          </>
        ) : (
